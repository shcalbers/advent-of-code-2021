    ...
    for (auto& line : lines) {
        if (is_horizontal(line)) {
            ...
            if (line.start.x <= line.end.x) {
                auto slope = line.start.y <= line.end.y ? 1 : -1;
                for (auto x = line.start.x, y = line.start.y; x <= line.end.x; x++, y += slope) {
                    if (++htvent_map[y][x] == 2) overlaps++;
                }
            } else {
                auto slope = line.end.y <= line.start.y ? 1 : -1;
                for (auto x = line.end.x, y = line.end.y; x <= line.start.x; x++, y += slope) {
                    if (++htvent_map[y][x] == 2) overlaps++;
                }
            }
        }
