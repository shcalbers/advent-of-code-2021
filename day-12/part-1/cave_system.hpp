...
public:
    ...
    auto find_paths(void) const -> std::vector<CavePath>;
...
private:
    ...
    auto find_paths(const CaveNode& current_node, CavePath current_path, std::vector<CavePath>& paths_out) const -> void;
...
inline auto CaveSystem::find_paths(void) const -> std::vector<CavePath> {
    std::vector<CavePath> paths;
    this->find_paths(*this->start, CavePath(), paths);

    return paths;
}

inline auto CaveSystem::find_paths(const CaveNode& current_node, CavePath current_path, std::vector<CavePath>& paths_out) const -> void {
    ...
    for (const auto& [id, next_node] : current_node.get_connected_nodes()) {
        if (next_node->is_small() && current_path.visited(*next_node)) continue;
        find_paths(*next_node, current_path, paths_out);
    }
}
